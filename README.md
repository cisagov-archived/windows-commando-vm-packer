...
AWS_PROFILE=cool-images-ec2amicreate-windows-commando-vm-packer packer build --timestamp-ui -var release_tag=$(./bump-version show) -var is_prerelease=true .
```
...
```console
AWS_PROFILE=cool-images-ec2amicreate-windows-commando-vm-packer packer build --timestamp-ui -var-file release.pkrvars.hcl .
```
...
| packages\_dir | The directory where the lists of packages are stored. | `string` | `"packages"` | no |
